      ...
      <div v-else class="streak-info">
        <div class="streak-card">
          <div class="streak-value">
            {{ profileData.challengeSummary?.currentStreak || 0 }}
          </div>
          <div class="streak-label">현재 스트릭</div>
        </div>
        <div class="streak-card">
          <div class="streak-value">
            {{ profileData.challengeSummary?.longestStreak || 0 }}
          </div>
            ...
            <div class="days-grid">
              ...
              <div
                ...
                :class="[
                  'day-cell',
                  isDateCompleted(
                    `${month.year}-${formatTwoDigits(
                      month.month
                    )}-${formatTwoDigits(date)}`
                  )
                    ? 'completed'
                    : '',
                  isToday(
...
const fetchProfileData = async () => {
  try {
    ...
    if (res.data) {
      ...
      // uploadedAt을 measureDate로 강제 매핑
      if (profileData.value.inbody && profileData.value.inbody.length > 0) {
        profileData.value.inbody = profileData.value.inbody.map((record) => ({
          ...record,
          measureDate: record.uploadedAt, // uploadedAt만 사용
        }));
      }
    }
  } catch (err) {
...
const sortedInbodyRecords = computed(() => {
  ...
  const records = [...profileData.value.inbody].map((record) => {
    // 각 레코드의 속성 확인 및 로깅
    console.log(
      `레코드 ID: ${record.id || "N/A"}, 측정일: ${
        record.measureDate || "날짜 없음"
      }`
    );
    ...
    return {
      ...
      bodyFatPercentage:
        record.bodyFatPercentage !== undefined ? record.bodyFatPercentage : 0,
  ...
  const sortedRecords = records.sort((a, b) => {
    ...
    try {
      // 문자열 형태면 Date 객체로 변환
      const dateA =
        typeof a.measureDate === "string"
          ? new Date(a.measureDate)
          : a.measureDate;
      const dateB =
        typeof b.measureDate === "string"
          ? new Date(b.measureDate)
          : b.measureDate;

      // 객체 형태인 경우 (LocalDate)
      if (
        typeof a.measureDate === "object" &&
        a.measureDate !== null &&
        !Array.isArray(a.measureDate) &&
        !(a.measureDate instanceof Date)
      ) {
...
const formatDate = (dateStr) => {
  ...
  if (
    ...
    try {
      ...
      if (
        dateStr.year !== undefined &&
        dateStr.monthValue !== undefined &&
        dateStr.dayOfMonth !== undefined
      ) {
        return `${dateStr.year}-${String(dateStr.monthValue).padStart(
          2,
          "0"
        )}-${String(dateStr.dayOfMonth).padStart(2, "0")}`;
      }
    } catch (e) {
...
const createWeightChart = () => {
  ...
  // 차트 데이터 준비
  const sortedData = [...profileData.value.inbody].sort(
    (a, b) => new Date(a.measureDate) - new Date(b.measureDate)
  );
...
onMounted(() => {
  ...
  fetchProfileData().then(() => {
    console.log(
      "streakCalendar:",
      profileData.value.challengeSummary.streakCalendar
    );
  });
});
</script>
