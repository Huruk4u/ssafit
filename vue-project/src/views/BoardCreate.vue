  ...
  <div class="board-create-container">
    ...
    <div class="form-container">
      ...
      <div class="button-group">
        ...
        <button class="submit-btn" @click="submitArticle" :disabled="isSubmitting">
...
const submitArticle = async () => {
  ...
  try {
    ...
    submitStatus.value = '오류 발생: ' + (error.message || JSON.stringify(error));
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        ...
        alert('잘못된 요청입니다: ' + (error.response.data || '요청 형식을 확인해주세요'));
      } else if (error.response.status === 500) {
        alert('서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.');
      } else {
        alert(`게시글 등록 중 오류가 발생했습니다(${error.response.status}). 다시 시도해주세요.`);
      }
...
const cancel = () => {
  const confirmLeave = confirm('작성 중인 내용은 저장되지 않습니다. 정말 취소하시겠습니까?');
  if (confirmLeave) {
    router.push('/board');
  }
};
